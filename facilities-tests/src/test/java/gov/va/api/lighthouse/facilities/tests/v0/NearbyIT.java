...
public class NearbyIT {
  ...
  @Test
  @SneakyThrows
  void searchByAddressAndLatLong() {
    final String streetAddress = systemDefinition().ids().streetAddress();
    final String city = systemDefinition().ids().city();
    final String state = systemDefinition().ids().state();
    final String zip = systemDefinition().ids().zip();
    ...
    final String searchByAddressFirstRequest =
        "v0/nearby?street_address="
            + streetAddress
            + "&city="
            + city
            ...
            + "&lng="
            + longitude;
    final String searchByLatLongFirstRequest =
        "v0/nearby?lat="
            + latitude
            + "&lng="
            + longitude
            ...
            + zip
            + "&drive_time=90";
    // Verify that search by address parameters are successfully ignored
    facilitiesRequest("application/json", searchByAddressFirstRequest, 200)
        .expectValid(NearbyResponse.class);
    facilitiesRequest("application/json", searchByLatLongFirstRequest, 200)
        .expectValid(NearbyResponse.class);
  }
